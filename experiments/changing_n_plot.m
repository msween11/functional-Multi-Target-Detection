...
for si = 1:numSeeds
    ...
    %l8 space errs
    l8_kot_space_err = zeros(1,length(nrange));
    l8_fm_space_err = zeros(1,length(nrange));
    l8_spec_space_err = zeros(1,length(nrange));

    %l2 space errs
    l2_kot_space_err = zeros(1,length(nrange));
    l2_fm_space_err = zeros(1,length(nrange));
    l2_spec_space_err = zeros(1,length(nrange));

    %l8 freq errs
    l8_kot_freq_err = zeros(1,length(nrange));
    l8_fm_freq_err = zeros(1,length(nrange));
    l8_spec_freq_err = zeros(1,length(nrange));

    %l2 freq errs
    l2_kot_freq_err = zeros(1,length(nrange));
    l2_fm_freq_err = zeros(1,length(nrange));
    l2_spec_freq_err = zeros(1,length(nrange));



    %start nrange loop
    for idx = 1:numel(nrange)
    
    %initialize parameters and grids
    [params, grids] = setparams(n = floor(2.^nrange(idx)), l = 2, B = 10, z = 1, ...
...
b8_kot = round(polyfit(nrange, log2(avg_table.l8_kot_space_err),1),2);
b8_fm = round(polyfit(nrange, log2(avg_table.l8_fm_space_err),1),2);
b8_spec = round(polyfit(nrange, log2(avg_table.l8_spec_space_err),1),2);

b2_kot = round(polyfit(nrange, log2(avg_table.l2_kot_space_err),1),2);
b2_fm = round(polyfit(nrange, log2(avg_table.l2_fm_space_err),1),2);
b2_spec = round(polyfit(nrange, log2(avg_table.l2_spec_space_err),1),2);
...
title('Changing n, space rel errors')

%tile 2----------------------------------------------
nexttile

%slopes of best fit lines
b8_kot = round(polyfit(nrange, log2(avg_table.l8_kot_freq_err),1),2);
b8_fm = round(polyfit(nrange, log2(avg_table.l8_fm_freq_err),1),2);
b8_spec = round(polyfit(nrange, log2(avg_table.l8_spec_freq_err),1),2);

b2_kot = round(polyfit(nrange, log2(avg_table.l2_kot_freq_err),1),2);
b2_fm = round(polyfit(nrange, log2(avg_table.l2_fm_freq_err),1),2);
b2_spec = round(polyfit(nrange, log2(avg_table.l2_spec_freq_err),1),2);

%plotting l8 errs
errorbar(nrange, log2(avg_table.l8_kot_freq_err), ...
    1.4427*errs.l8_kot_freq_err ./ avg_table.l8_kot_freq_err, 'red')
hold on
errorbar(nrange, log2(avg_table.l8_fm_freq_err), ...
    1.4427*errs.l8_fm_freq_err ./ avg_table.l8_fm_freq_err,'blue')
errorbar(nrange, log2(avg_table.l8_spec_freq_err), ...
    1.4427*errs.l8_spec_freq_err ./ avg_table.l8_spec_freq_err,'green')

%plotting l2 errs
errorbar(nrange, log2(avg_table.l2_kot_freq_err), ...
    1.4427*errs.l2_kot_freq_err ./ avg_table.l2_kot_freq_err, 'Color','red','LineStyle' ,'--')
hold on
errorbar(nrange, log2(avg_table.l2_fm_freq_err), ...
    1.4427*errs.l2_fm_freq_err ./ avg_table.l2_fm_freq_err,'Color','blue','LineStyle' ,'--')
errorbar(nrange, log2(avg_table.l2_spec_freq_err), ...
...
legend(['Kotlarski l8' ' m = ', num2str(b8_kot(1))],['FM l8' ' m = ', num2str(b8_fm(1))],...
    ...
    ['FM l2' ' m = ', num2str(b2_fm(1))],...
    ['Spectral l2' ' m = ', num2str(b2_spec(1))])
title('Changing n, freq. rel errors')

%saving the figure
filename = sprintf('figures/changing_n_%s.fig', func_choice); 
