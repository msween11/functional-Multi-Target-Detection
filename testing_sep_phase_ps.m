...
[params, grids] = setparams(n= 2^10, l = 5, B = 10, z = 1, ...
    t = .01, sigma = .05, lambda = .1, r = .1, h = .05);
...
% [recspace, recfft, recspace_test, recfft_test] = kotlarski_simps_test(params, grids, psihat, ps);
...
%ps is just A2M, normalized and forced positive
...
phase_deriv = imag(integrand_reg);
phase = cumtrapz(phase_deriv)*grids.dw;
phase = phase - phase(grids.Wext == 0);
