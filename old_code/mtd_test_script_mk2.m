...
try
    ...
    if strcmp(ME.identifier,'MATLAB:array:SizeLimitExceeded')
        ...
        UL = zeros(dd,dd); UR = zeros(dd,dd); 
        DD = D(1:dd); DD2 = D(dd+1:end);                
    ...
    for i = 1:dd
        ...
        for j = 1:i
            jj = DD2(j)*2^l;
            LR(i,j) = 1/n*2^(-l)*dot(di,datastrip(d+jj:end-d-1+jj));
            LR(j,i) = LR(i,j);
        end
    end
        test = [UL UR ; UR' rot90(UL,2)]  ;   
        % figure
        % imagesc(UR); title('UR')
        % figure; imagesc(UL); title('UL')

        %issue is that A3M diag is symmetric across entry 65, and test diag
        %is symmetric around entry 64=65, it's like we're missing entry 65
...
for j = 1:length(Dext)
    FM_space(j) = (wgap/(2*pi)).* sum(exp(1i.*Wext*Dext(j)).*...
        FM_freq .* phiK(h*Wext));
end
