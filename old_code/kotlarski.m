function [recspace, recfft] = kotlarski(params, grids, psihat, psireg)

%% DOING KOTLARSKI NOW

%TO DO: add simps beyond the derivative steps
...
rec_emp = zeros(1,length(grids.Wext)/2);
rec_emp(1) = 0;
for j = 2:length(grids.Wext)/2 
    rec_emp(j) = rec_emp(j-1) + ...
        grids.dw*integrand_emp(length(grids.Wext)/2 +j);
end
rec_emp = exp(rec_emp);
...
rec_reg = zeros(1,length(grids.Wext)/2);
rec_reg(1) = 0;
for j = 2:length(grids.Wext)/2 
    rec_reg(j) = rec_reg(j-1) + grids.dw*integrand_reg( length(grids.Wext)/2 +j);
end

rec1_reg = (exp(rec_reg)); 
rec1_reg = rec_emp; %only for discretization
...
for j = 1:length(grids.Dext) 
    rec(j) = (grids.dw/(2*pi)).* simps(exp(1i.*grids.Wext*grids.Dext(j)).*...
